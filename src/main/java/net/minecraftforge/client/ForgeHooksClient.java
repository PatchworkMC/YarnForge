...
*/
    ...
    public static void renderMainMenu(TitleScreen gui, MatrixStack mStack, TextRenderer font, int width, int height)
    {
        VersionChecker.Status status = ForgeVersion.getStatus();
        if (status == BETA || status == BETA_OUTDATED)
        {
            // render a warning at the top of the screen,
            Text line = new TranslatableText("forge.update.beta.1", Formatting.RED, Formatting.RESET).formatted(Formatting.RED);
            DrawableHelper.drawCenteredText(mStack, font, line, width / 2, 4 + (0 * (font.fontHeight + 1)), -1);
            line = new TranslatableText("forge.update.beta.2");
            DrawableHelper.drawCenteredText(mStack, font, line, width / 2, 4 + (1 * (font.fontHeight + 1)), -1);
        }
    ...
    public static Sprite[] getFluidSprites(BlockRenderView world, BlockPos pos, FluidState fluidStateIn)
    {
        ...
        return new Sprite[] {
                MinecraftClient.getInstance().getSpriteAtlas(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE).apply(fluidStateIn.getFluid().getAttributes().getStillTexture(world, pos)),
                MinecraftClient.getInstance().getSpriteAtlas(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE).apply(fluidStateIn.getFluid().getAttributes().getFlowingTexture(world, pos)),
                overlayTexture == null ? null : MinecraftClient.getInstance().getSpriteAtlas(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE).apply(overlayTexture),
        };
    }
    ...
    {
        return new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, loc);
    }
    ...
    public static RenderGameOverlayEvent.BossInfo bossBarRenderPre(MatrixStack mStack, Window res, ClientBossBar bossInfo, int x, int y, int increment)
    {
    ...
    public static void bossBarRenderPost(MatrixStack mStack, Window res)
    {
    ...
    public static void onClientChangeGameMode(PlayerListEntry info, GameMode currentGameMode, GameMode newGameMode)
    {
    ...
    {
        AffineTransformation tr = TransformationHelper.toTransformation(model.getTransformation().getTransformation(type));
        if(!tr.isIdentity()) {
            tr.push(stack);
        }
        return model;
    ...
    public static void drawItemLayered(ItemRenderer renderer, BakedModel modelIn, ItemStack itemStackIn, MatrixStack matrixStackIn,
                                       VertexConsumerProvider bufferIn, int combinedLightIn, int combinedOverlayIn, boolean fabulous)
    {
        for(com.mojang.datafixers.util.Pair<BakedModel,RenderLayer> layerModel : modelIn.getLayerModels(itemStackIn, fabulous))
        {
            BakedModel layer = layerModel.getFirst();
            RenderLayer rendertype = layerModel.getSecond();
            net.minecraftforge.client.ForgeHooksClient.setRenderLayer(rendertype); // neded for compatibility with MultiLayerModels
            VertexConsumer ivertexbuilder;
            if (fabulous)
            {
                ivertexbuilder = ItemRenderer.getDirectItemGlintConsumer(bufferIn, rendertype, true, itemStackIn.hasGlint());
            } else {
                ivertexbuilder = ItemRenderer.getItemGlintConsumer(bufferIn, rendertype, true, itemStackIn.hasGlint());
            }
            renderer.renderBakedItemModel(layer, itemStackIn, combinedLightIn, combinedOverlayIn, matrixStackIn, ivertexbuilder);
        }
    ...
    public static boolean isNameplateInRenderDistance(Entity entity, double squareDistance) {
        if (entity instanceof LivingEntity) {
            final EntityAttributeInstance attribute = ((LivingEntity) entity).getAttributeInstance(ForgeMod.NAMETAG_DISTANCE.get());
            if (attribute != null) {
    ...
    public static void renderPistonMovedBlocks(BlockPos pos, BlockState state, MatrixStack stack, VertexConsumerProvider buffer, World world, boolean checkSides, int combinedOverlay, BlockRenderManager blockRenderer) {
        RenderLayer.getBlockLayers().stream()
                .filter(t -> RenderLayers.canRenderInLayer(state, t))
                .forEach(rendertype ->
                {
                    rendertype = rendertype == RenderLayer.getTranslucent() ? RenderLayer.getTranslucentMovingBlock() : rendertype;
                    setRenderLayer(rendertype);
                    VertexConsumer ivertexbuilder = buffer.getBuffer(rendertype);
                    blockRenderer.getModelRenderer().render(world, blockRenderer.getModel(state), state, pos, stack, ivertexbuilder, checkSides, new Random(), state.getRenderingSeed(pos), combinedOverlay);
                });
        setRenderLayer(null);
    }
    ...
    public static GeneratorType.ScreenProvider getBiomeGeneratorTypeScreenFactory(Optional<GeneratorType> generator, @Nullable GeneratorType.ScreenProvider biomegeneratortypescreens$ifactory)
    {
        return ForgeWorldTypeScreens.getGeneratorScreenFactory(generator, biomegeneratortypescreens$ifactory);
    }

    public static boolean hasBiomeGeneratorSettingsOptionsScreen(Optional<GeneratorType> generator)
    {
        return getBiomeGeneratorTypeScreenFactory(generator, null) != null;
    }

    public static Optional<GeneratorType> getWorldTypeFromGenerator(GeneratorOptions dimensionGeneratorSettings)
    {
        return GeneratorType.method_29078(dimensionGeneratorSettings);
    }

    public static Optional<GeneratorType> getDefaultWorldType()
    {
