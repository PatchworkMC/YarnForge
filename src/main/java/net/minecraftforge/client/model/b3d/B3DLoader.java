...
public enum B3DLoader implements ISelectiveResourceReloadListener
{
    ...
    public static final class B3DState implements ModelBakeSettings
    {
        ...
        public AffineTransformation getPartTransformation(Object part)
        {
            ...
            AffineTransformation nodeTransform;
        ...
        private static LoadingCache<Triple<Animation, Node<?>, Integer>, AffineTransformation> cache = CacheBuilder.newBuilder()
            .maximumSize(16384)
            .expireAfterAccess(2, TimeUnit.MINUTES)
            .build(new CacheLoader<Triple<Animation, Node<?>, Integer>, AffineTransformation>()
            {
                @Override
                public AffineTransformation load(Triple<Animation, Node<?>, Integer> key) throws Exception
                {
                    return getNodeMatrix(key.getLeft(), key.getMiddle(), key.getRight());
                }
            });

        public AffineTransformation getNodeMatrix(Node<?> node)
        {
            return getNodeMatrix(node, frame);
        }

        public AffineTransformation getNodeMatrix(Node<?> node, int frame)
        {
            return cache.getUnchecked(Triple.of(animation, node, frame));
        }

        public static AffineTransformation getNodeMatrix(@Nullable Animation animation, Node<?> node, int frame)
        {
            AffineTransformation ret = AffineTransformation.identity();
            Key key = null;
            ...
            if(key != null)
            {
                ...
                if(parent != null)
                {
                    ...
                    AffineTransformation pm = cache.getUnchecked(Triple.of(animation, node.getParent(), frame));
                    ret = ret.compose(pm);
                    // joint offset in the parent coords
                    ret = ret.compose(new AffineTransformation(parent.getPos(), parent.getRot(), parent.getScale(), null));
                }
                // current node local pose
                ret = ret.compose(new AffineTransformation(key.getPos(), key.getRot(), key.getScale(), null));
                ...
                // TODO cache
                AffineTransformation invBind = new NodeJoint(node).getInvBindPose();
            ...
            else
            {
                ...
                ret = ret.compose(new AffineTransformation(node.getPos(), node.getRot(), node.getScale(), null));
                // TODO cache
                AffineTransformation invBind = new NodeJoint(node).getInvBindPose();
                ret = ret.compose(invBind);
            }
    ...
    static final class NodeJoint implements IJoint
    {
        ...
        @Override
        public AffineTransformation getInvBindPose()
        {
            Matrix4f m = new AffineTransformation(node.getPos(), node.getRot(), node.getScale(), null).getMatrix();
            m.invert();
            AffineTransformation pose = new AffineTransformation(m);

            if(node.getParent() != null)
            {
                AffineTransformation parent = new NodeJoint(node.getParent()).getInvBindPose();
    ...
    private static final class ModelWrapper implements UnbakedModel
    {
        ...
        public Collection<SpriteIdentifier> getTextureDependencies(Function<Identifier, UnbakedModel> modelGetter, Set<com.mojang.datafixers.util.Pair<String, String>> missingTextureErrors)
        {
            return textures.values().stream().filter(loc -> !loc.startsWith("#"))
                    .map(t -> new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, new Identifier(t)))
        ...
        public BakedModel bake(ModelLoader bakery, Function<SpriteIdentifier, Sprite> spriteGetter, ModelBakeSettings modelTransform, Identifier modelLocation)
        {
            ...
            Sprite missing = spriteGetter.apply(new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, MissingSprite.getMissingSpriteId()));
            for(Map.Entry<String, String> e : textures.entrySet())
            {
                ...
                {
                    builder.put(e.getKey(), spriteGetter.apply(new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, new Identifier(e.getValue()))));
                }
            }
    ...
    private static final class BakedWrapper implements IDynamicBakedModel
    {
        ...
        private void generateQuads(ImmutableList.Builder<BakedQuad> builder, Node<?> node, final ModelBakeSettings state, ImmutableList<String> path)
        {
            ...
            if(node.getKind() instanceof Mesh && meshes.contains(node.getName()) && state.getPartTransformation(Models.getHiddenModelPart(newPath)).isIdentity())
            {
                ...
                {
                    private final AffineTransformation global = state.getRotation();
                    private final LoadingCache<Node<?>, AffineTransformation> localCache = CacheBuilder.newBuilder()
                        .maximumSize(32)
                        .build(new CacheLoader<Node<?>, AffineTransformation>()
                        {
                            @Override
                            public AffineTransformation load(Node<?> node) throws Exception
                            {
