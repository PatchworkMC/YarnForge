...
public enum B3DClip implements IClip
{
    ...
    protected static class NodeClip implements IJointClip
    {
        ...
        @Override
        public AffineTransformation apply(float time)
        {
            AffineTransformation ret = AffineTransformation.identity();
            if(node.getAnimation() == null)
            {
                return ret.compose(new AffineTransformation(node.getPos(), node.getRot(), node.getScale(), null));
            }
            ...
            AffineTransformation startTr = keyStart == null ? null : new AffineTransformation(keyStart.getPos(), keyStart.getRot(),keyStart.getScale(), null);
            AffineTransformation endTr = keyEnd == null ? null : new AffineTransformation(keyEnd.getPos(), keyEnd.getRot(),keyEnd.getScale(), null);
            if(keyStart == null)
            {
                if(keyEnd == null)
                {
