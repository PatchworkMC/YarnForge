...
public class ForgeHooks
{
    ...
    public static Text newChatWithLinks(String string, boolean allowMissingHeader)
    {
        ...
        while (matcher.find())
        {
            ...
            link.setStyle(link.getStyle().withClickEvent(click).setUnderlined(true).withColor(TextColor.fromFormatting(Formatting.BLUE)));
            if (ichat == null)
        ...
        if (ichat == null)
            ...
            ichat.append(new LiteralText(string.substring(lastEnd)));
        return ichat;
    }
        ...
        if (ret.isAccepted())
        {
            ...
            else
            {
                ...
                for (BlockSnapshot snap : blockSnapshots)
                {
                    ...
                    world.markAndNotifyBlock(snap.getPos(), world.getWorldChunk(snap.getPos()), oldBlock, newBlock, updateFlag, 512);
                }
    ...
    public static boolean onAnvilChange(AnvilScreenHandler container, @Nonnull ItemStack left, @Nonnull ItemStack right, Inventory outputSlot, String name, int baseCost, PlayerEntity player)
    {
    ...
    public static boolean onChangeGameMode(PlayerEntity player, GameMode currentGameMode, GameMode newGameMode)
    {
    ...
    {
        Biome apply(final Biome.Weather climate, final Biome.Category category, final Float depth, final Float scale, final BiomeEffects effects, final GenerationSettings gen, final SpawnSettings spawns);
    }

    public static Biome enhanceBiome(@Nullable final Identifier name, final Biome.Weather climate, final Biome.Category category, final Float depth, final Float scale, final BiomeEffects effects, final GenerationSettings gen, final SpawnSettings spawns, final RecordCodecBuilder.Instance<Biome> codec, final BiomeCallbackFunction callback)
    {
    ...
    public static <T> void deserializeTagAdditions(List<Tag.Entry> list, JsonObject json, List<Tag.TrackedEntry> allList)
    {
        ...
        if (json.has("optional"))
        {
            for (JsonElement entry : JsonHelper.getArray(json, "optional"))
            {
                ...
                if (!s.startsWith("#"))
                    list.add(new Tag.OptionalObjectEntry(new Identifier(s)));
                else
                    list.add(new Tag.OptionalTagEntry(new Identifier(s.substring(1))));
            }
        }
        ...
        if (json.has("remove"))
        {
            for (JsonElement entry : JsonHelper.getArray(json, "remove"))
            {
                ...
                Tag.Entry dummy;
                if (!s.startsWith("#"))
                    dummy = new Tag.ObjectEntry(new Identifier(s));
                else
                    dummy = new Tag.TagEntry(new Identifier(s.substring(1)));
    ...
    {
        int id = vanilla.getRawId(serializer);
        if (id < 0)
        {
     ...
     */
    public static void fixNullStructureReferences(Chunk chunk, Map<StructureFeature<?>, LongSet> structureReferences)
    {
